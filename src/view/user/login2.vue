  ...
  export default {
    ...
    methods: {
      ...
      submit(item) {
        ...
        // 网络请求模块
        this.$http.post("token/CheckLogin", {
          ...
          if (response.body.code == 0) { 
            ...
            // 存入token
            localStorage.setItem('token', response.body.data.token.token);
            localStorage.setItem('express', response.body.data.token.express); 

            that.$store.commit('updateToken', response.body.data.token);  
